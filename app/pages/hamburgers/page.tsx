      ...
      <div className="flex gap-4">
        {hamburgers.map((hamburger: Hamburger) => (
          <ProductCard key={hamburger.id}>
            <ProductCardHeader>
              <ProductCardImage src={hamburger.image[0]} alt={hamburger.title} width={120} height={120} />
            <ProductCardDescription>
            ...
            <ProductCardAction>
              {hamburger.values.combo ? (
                <>
                  <ProductRadioButtom id={`${hamburger.id}-${hamburger.values.single}`}
                  label="Unico"
                  name={hamburger.title}
                  onChange={handleChange}
                  value={hamburger.values.single} />

                  <ProductRadioButtom id={`${hamburger.id}-${hamburger.values.combo}`}
                    label="Combo"
                    name={hamburger.title}
                    onChange={handleChange}
