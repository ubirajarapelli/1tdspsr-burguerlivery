...
import { Hamburger, listaHamburger } from "@/app/types/hamburger"
...
export default function Hamburgers() {
  ...
  const [hamburguers, setHamburgers] = useState<listaHamburger>([])
  ...
  const handleClick = (id: number) => {
    if (productValue === 0) {
      return console.log("nada aqui em hamburger")
    }

    const selectHamburger = hamburguers.filter((hamburger) => hamburger.id === id) 
  ...
  <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {hamburguers.map((hamburger: Hamburger) => (
